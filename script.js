document.addEventListener("DOMContentLoaded", () => {
    ...
    const stakeIdInput = document.getElementById("stake-id");
    ...
    stakeIdInput.addEventListener("input", validateForm);
...
function validateAmount() {
    ...
    if (isNaN(amount)) {
        ...
        amountMessage.textContent = "Amount must be $50 or more";
        amountMessage.className = "amount-message error";
        return false;
    } else {
        amountMessage.textContent = `You will receive ${(amount * 2).toFixed(2)}$`;
...
function validateForm() {
    ...
    const isFormValid = stakeId && walletAddress && amount && screenshot > 0 && cryptoSelect;
    document.querySelector(".submit-btn").disabled = !isFormValid;
}